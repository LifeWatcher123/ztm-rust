...
fn authorize(
    ...
    //     1. Connect to the database
    // For future me, the question mark `returns` an `Err` enum
    // which terminates the enclosing function.
    ...
    if keycard.access_level >= location.required_access_level() {
        // Crash course reminds that `Result::` part can be omitted,
        // similar to the `Option` enum.
        Result::Ok(AuthorizationStatus::Allow)
    } else {
        Result::Ok(AuthorizationStatus::Deny)
    }
